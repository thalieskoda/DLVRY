  ...
  const CheckIfLocationEnabled = async () => {
    ...
    if (!enabled) {
      Alert.alert(
        ...
        "Please enable your Location service to continue"[{ Text: "OK" }],
        { cancelable: false }
      );
    } else {
      setLocationServiceEnable(true);
    }
  };
  const GetCurrentLocation = async () => {
    ...
    if (status !== granted) {
      Alert.alert(
        ...
        "Allow the app to use the location service"[{ Text: "OK" }],
        { cancelable: false }
      );
    }
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: "#f8f8f8" }}>
      <View
        ...
        <Ionicons name="location" size={24} color="#e52850" />
